...
export type GasFuturesAutoConfig = {
  pkg: string;
  marketId: string;
  feeVaultId: string;
  maxVictims?: number;
...
export function createGasFuturesAutoKeeper(client: SuiClient, sender: string, exec: TxExecutor, cfg: GasFuturesAutoConfig): Keeper {
  ...
  async function step(): Promise<void> {
    ...
    for (const r of rows) {
      ...
      tx.moveCall({ target: `${pkg}::gas_futures::liquidate`, arguments: [tx.object(cfg.marketId), tx.pure.address(victim), tx.pure.u64(qty), tx.object(cfg.feeVaultId), tx.object('0x6'), tx.object('0x6')] });
