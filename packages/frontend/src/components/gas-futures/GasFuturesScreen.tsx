...
export function GasFuturesScreen({ started, surgeReady, network, protocolStatus }: { 
  started?: boolean; 
  surgeReady?: boolean; 
  network?: string;
  protocolStatus?: {
  ...
  const dataProvider = useMemo(() => createMockDerivativesProvider('gas-futures'), []);
  const panelProvider = useMemo(() => createMockTradePanelProvider(), []);
  const availableExpiries = useMemo(() => createMockExpiryContracts('gas-futures'), []);
