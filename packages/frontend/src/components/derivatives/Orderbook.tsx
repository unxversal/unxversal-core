...
import type { DerivativesDataProvider, OrderbookSnapshot } from './types';

export function Orderbook({ provider }: { provider?: DerivativesDataProvider }) {
    ...
    const load = async () => {
      try {
        if (provider?.getOrderbook) {
          const ob: OrderbookSnapshot = await provider.getOrderbook();
          if (!mounted) return;
          setBids(ob.bids);
          setAsks(ob.asks);
          return;
