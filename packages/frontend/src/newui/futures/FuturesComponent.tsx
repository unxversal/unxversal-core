...
import type { FuturesComponentProps, Candle } from './types';
...
export function FuturesComponent(props: FuturesComponentProps) {
  const { address, network, protocolStatus, marketLabel, markets = [], selectedMarketId, onSelectMarket, symbol, quoteSymbol = 'USDC', expiries = [], onExpiryChange, summary = {}, ohlc, orderbook, recentTrades = [], positions = [], openOrders = [], twap = [], TradePanelComponent } = props;
      ...
      <div className={styles.priceCard}>
        <div className={styles.pairBar}>
          <div className={styles.pair}>
            {markets.length > 0 ? (
              <select
                ...
                value={selectedMarketId || markets[0]?.marketId || ''}
                onChange={(e) => onSelectMarket?.(e.target.value)}
              >
                {markets.map(m => (
                  <option key={m.marketId} value={m.marketId}>{m.label || m.symbol}</option>
                ))}
              </select>
            ) : (
              marketLabel
            ...
            {expiries.length > 0 && (
              <select
                className={styles.expirySelector}
                value={expiries.find(e => e.isActive)?.id || expiries[0]?.id || ''}
          ...
          <div className={styles.metrics}>
            <div className={styles.metricItem}><div className={styles.metricValue}>{summary.last?.toFixed(4) ?? '-'}</div><div className={styles.metricLabel}>Price</div></div>
            ...
            {ohlc?.candles?.length ? (
              <div className={styles.ohlcDisplay}>
                ...
                {(() => { const c = ohlc.candles.at(-1)!; return (
                  <>
                    <span className={styles.ohlcValue}>O {c.open.toFixed(4)}</span>
                    <span className={styles.ohlcValue}>H {c.high.toFixed(4)}</span>
                    <span className={styles.ohlcValue}>L {c.low.toFixed(4)}</span>
                    <span className={styles.ohlcValue}>C {c.close.toFixed(4)}</span>
                  </>
                ); })()}
              </div>
            ) : null}
            ...
            <table className={styles.ordersTable}>
              ...
              <tbody>
                {(orderbook?.bids || []).slice(0, 20).map(([p, q], i) => (
                  <tr key={`b-${i}`}><td className={styles.longText}>{p.toFixed(6)}</td><td>{q}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
            ...
            <table className={styles.ordersTable}>
              ...
              <tbody>
                {(orderbook?.asks || []).slice(0, 20).map(([p, q], i) => (
                  <tr key={`a-${i}`}><td className={styles.shortText}>{p.toFixed(6)}</td><td>{q}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
