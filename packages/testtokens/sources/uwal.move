/// Module: testtokens_uwal (Testnet token + USDU-priced faucet)
/// - Symbol: UWAL, 6 decimals. Price from Pyth symbol "WAL/USDC".
module testtokens::uwal {
    ...
    public struct UWAL has drop {}
    public struct Faucet has key, store { id: UID, cap: TreasuryCap<UWAL>, usdu_treasury: Balance<USDU>, paused: bool }
    ...
    public struct BoughtWithUSDU has copy, drop { buyer: address, usdu_in: u64, uwal_out: u64, price_1e6: u64, timestamp_ms: u64 }

    fun init(witness: UWAL, ctx: &mut TxContext) {
        let (cap, metadata) = coin::create_currency(witness, 6, b"UWAL", b"Unxversal Testnet WAL", b"", option::none(), ctx);
    ...
    public fun buy_with_usdu(reg: &OracleRegistry, faucet: &mut Faucet, price_info_object: &PriceInfoObject, usdu_in: Coin<USDU>, min_out: u64, clock: &Clock, ctx: &mut TxContext): Coin<UWAL> {
        assert!(!faucet.paused, E_PAUSED);
        let price_1e6 = oracle::get_price_for_symbol(reg, clock, &symbol_wal_usdc(), price_info_object);
        ...
        event::emit(BoughtWithUSDU { buyer: ctx.sender(), usdu_in: usdu_amount, uwal_out: out, price_1e6, timestamp_ms: sui::clock::timestamp_ms(clock) });
        minted
    }
    ...
    fun symbol_wal_usdc(): String { string::utf8(b"WAL/USDC") }
}
