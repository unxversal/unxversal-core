/// Module: testtokens_umatic (Testnet token + USDU-priced faucet)
/// - Symbol: UMATIC, 6 decimals. Price from Pyth symbol "MATIC/USDC".
module testtokens::umatic {
    ...
    public struct UMATIC has drop {}
    public struct Faucet has key, store { id: UID, cap: TreasuryCap<UMATIC>, usdu_treasury: Balance<USDU>, paused: bool }
    ...
    public struct BoughtWithUSDU has copy, drop { buyer: address, usdu_in: u64, umatic_out: u64, price_1e6: u64, timestamp_ms: u64 }

    fun init(witness: UMATIC, ctx: &mut TxContext) {
        let (cap, metadata) = coin::create_currency(witness, 6, b"UMATIC", b"Unxversal Testnet MATIC", b"", option::none(), ctx);
    ...
    public fun buy_with_usdu(reg: &OracleRegistry, faucet: &mut Faucet, price_info_object: &PriceInfoObject, usdu_in: Coin<USDU>, min_out: u64, clock: &Clock, ctx: &mut TxContext): Coin<UMATIC> {
        assert!(!faucet.paused, E_PAUSED);
        let price_1e6 = oracle::get_price_for_symbol(reg, clock, &symbol_matic_usdc(), price_info_object);
        ...
        event::emit(BoughtWithUSDU { buyer: ctx.sender(), usdu_in: usdu_amount, umatic_out: out, price_1e6, timestamp_ms: sui::clock::timestamp_ms(clock) });
        minted
    }
    ...
    fun symbol_matic_usdc(): String { string::utf8(b"MATIC/USDC") }
}
