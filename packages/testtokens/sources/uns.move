/// Module: testtokens_uns (Testnet token + USDU-priced faucet)
/// - Symbol: UNS, 6 decimals. Price from Pyth symbol "NS/USDC".
module testtokens::uns {
    ...
    public struct UNS has drop {}
    public struct Faucet has key, store { id: UID, cap: TreasuryCap<UNS>, usdu_treasury: Balance<USDU>, paused: bool }
    ...
    public struct BoughtWithUSDU has copy, drop { buyer: address, usdu_in: u64, uns_out: u64, price_1e6: u64, timestamp_ms: u64 }

    fun init(witness: UNS, ctx: &mut TxContext) {
        let (cap, metadata) = coin::create_currency(witness, 6, b"UNS", b"Unxversal Testnet NS", b"", option::none(), ctx);
    ...
    public fun buy_with_usdu(reg: &OracleRegistry, faucet: &mut Faucet, price_info_object: &PriceInfoObject, usdu_in: Coin<USDU>, min_out: u64, clock: &Clock, ctx: &mut TxContext): Coin<UNS> {
        assert!(!faucet.paused, E_PAUSED);
        let price_1e6 = oracle::get_price_for_symbol(reg, clock, &symbol_ns_usdc(), price_info_object);
        ...
        event::emit(BoughtWithUSDU { buyer: ctx.sender(), usdu_in: usdu_amount, uns_out: out, price_1e6, timestamp_ms: sui::clock::timestamp_ms(clock) });
        minted
    }
    ...
    fun symbol_ns_usdc(): String { string::utf8(b"NS/USDC") }
}
