module unxversal::options {
    ...
    *******************************/
    ...
    use sui::clock::Clock;
    // Production-ready oracle integration (no pyth dependencies)
    ...
    use unxversal::synthetics::{Self as Synth, SynthRegistry, AdminCap, DaddyCap, CollateralConfig};
    ...
    * Underlying & Registry
    *******************************/
    public struct UnderlyingAsset has store, drop {
        asset_name: String,
    ...
    *******************************/
    ...
    public struct ShortUnderlyingEscrow<phantom Base: store> has key, store {
        id: UID,
        position_id: ID,
    ...
    public struct CoinShortOffer<phantom Base: store> has key, store {
        id: UID,
        owner: address,
        market_id: ID,
        option_type: String,
    ...
    *******************************/
    ...
    fun assert_is_admin_via_synth(synth_reg: &SynthRegistry, addr: address) { assert!(unxversal::synthetics::check_is_admin(synth_reg, addr), E_NOT_ADMIN); }

    /*******************************
    * INIT - executed once on package publish
    *******************************/
    fun init(otw: OPTIONS, ctx: &mut tx_context::TxContext) {
        ...
        let reg = OptionsRegistry {
            ...
            paused: false,
            treasury_id: object::id_from_address(@0x0), // TODO: Set proper treasury ID during deployment
            trade_fee_bps: 30,
        ...
        sui::transfer::public_transfer(disp_reg, tx_context::sender(ctx));
    ...
    *******************************/
    public fun add_underlying(
        ...
        assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx));
        assert!(!reg.paused, E_PAUSED);
        assert!(!table::contains(&reg.supported_underlyings, symbol), E_UNDERLYING_EXISTS);
        table::add(&mut reg.supported_underlyings, symbol, asset);
        event::emit(UnderlyingAdded { symbol, by: tx_context::sender(ctx), timestamp: 0u64 });
    }
    ...
    public entry fun remove_underlying(
        ...
        table::remove(&mut reg.supported_underlyings, symbol);
        event::emit(UnderlyingRemoved { symbol, by: tx_context::sender(ctx), timestamp: 0u64 });
    }

    public entry fun pause(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.paused = true; event::emit(PausedToggled { new_state: true, by: tx_context::sender(ctx), timestamp: 0u64 }); }
    public entry fun resume(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.paused = false; event::emit(PausedToggled { new_state: false, by: tx_context::sender(ctx), timestamp: 0u64 }); }

    public entry fun set_treasury<C>(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, treasury: &Treasury<C>, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.treasury_id = object::id(treasury); }
    public entry fun set_fee_config(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, trade_fee_bps: u64, unxv_discount_bps: u64, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.trade_fee_bps = trade_fee_bps; reg.unxv_discount_bps = unxv_discount_bps; }
    public entry fun set_settlement_fee_bps(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, bps: u64, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.settlement_fee_bps = bps; }
    public entry fun set_liq_penalty_bps(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, bps: u64, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.liq_penalty_bps = bps; }
    public entry fun set_bot_reward_bps(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, liq_bot_bps: u64, close_bot_bps: u64, settle_bot_bps: u64, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.liq_bot_reward_bps = liq_bot_bps; reg.close_bot_reward_bps = close_bot_bps; reg.settlement_bot_reward_bps = settle_bot_bps; }
    public entry fun set_maker_rebate_bps_close(_admin: &AdminCap, synth_reg: &SynthRegistry, reg: &mut OptionsRegistry, bps: u64, ctx: &tx_context::TxContext) { assert_is_admin_via_synth(synth_reg, tx_context::sender(ctx)); reg.maker_rebate_bps_close = bps; }
    ...
    public entry fun grant_admin_via_synth(daddy: &DaddyCap, synth_reg: &mut SynthRegistry, new_admin: address, ctx: &mut tx_context::TxContext) { Synth::grant_admin(daddy, synth_reg, new_admin, ctx); }
    ...
    *******************************/
    public entry fun create_option_market<C>(
        ...
        let market = OptionMarket {
            ...
            is_active: true,
            is_expired: false,
            exercise_style: reg.default_exercise_style,
        ...
        table::add(&mut reg.option_markets, key_bytes, mid);

        // Optional creation fee collection with UNXV discount at source
        if (creation_fee_usdc > 0) {
            // Apply UNXV discount
            let discount_usdc = (creation_fee_usdc * reg.unxv_discount_bps) / 10_000;
            ...
            if (discount_usdc > 0 && std::vector::length(&unxv_payment) > 0) {
                ...
                // For now, accept UNXV as is and deposit; creation fee USDC reduced accordingly
                ...
                if (merged.value() > 0) {
                    let mut vecu = std::vector::empty<Coin<UNXV>>();
                    std::vector::push_back(&mut vecu, merged);
                    TreasuryMod::deposit_unxv(treasury, vecu, b"options_market_create".to_string(), tx_context::sender(ctx), ctx);
                    discount_applied = true;
                } else {
                    coin::destroy_zero(merged);
                };
            };
            let usdc_due = if (discount_applied && creation_fee_usdc > discount_usdc) { creation_fee_usdc - discount_usdc } else { creation_fee_usdc };
            if (usdc_due > 0) {
                let pay = creation_fee_coin.split(usdc_due, ctx);
                sui::transfer::public_transfer(pay, TreasuryMod::treasury_address(treasury));
            };
            // refund any remainder
            sui::transfer::public_transfer(creation_fee_coin, tx_context::sender(ctx));
        } else {
        ...
        event::emit(OptionMarketCreated { market_id: mid, market_key_bytes: key_bytes, underlying, option_type, strike_price, expiry_ms, settlement_type, creator: tx_context::sender(ctx), timestamp: 0u64 });
    }

    public fun init_settlement_queue(dispute_window_ms: u64, ctx: &mut tx_context::TxContext): SettlementQueue {
        SettlementQueue { id: object::new(ctx), dispute_window_ms, pending: std::vector::empty<ID>(), requested_at_ms: table::new<ID, u64>(ctx) }
    }

    public entry fun request_market_settlement(queue: &mut SettlementQueue, market: &OptionMarket, _ctx: &mut tx_context::TxContext) {
        std::vector::push_back(&mut queue.pending, object::id(market));
        table::add(&mut queue.requested_at_ms, object::id(market), 0u64);
    }

    public fun process_due_settlements(queue: &mut SettlementQueue, _reg: &OptionsRegistry, _oracle_cfg: &OracleConfig, _ctx: &mut tx_context::TxContext) {
        let now = 0u64;
        let mut i = 0;
        while (i < std::vector::length(&queue.pending)) {
            let market_id = *std::vector::borrow(&queue.pending, i);
            // TODO: Need proper market/clock/price references
            // let mref = get_market_by_id(market_id);
            // let clk = default_clock;
            // let p = default_price;
            if (table::contains(&queue.requested_at_ms, market_id)) {
                let t = *table::borrow(&queue.requested_at_ms, market_id);
                if (now >= t + queue.dispute_window_ms) {
                    // TODO: Settlement needs proper market reference
                    // expire_and_settle_market_cash(reg, mref, oracle_cfg, clk, p, ctx);
                    table::remove(&mut queue.requested_at_ms, market_id);
                }
            };
            i = i + 1;
        };
        // Rebuild pending vector excluding already processed
        let mut new_pending = std::vector::empty<ID>();
        let mut j = 0;
        while (j < std::vector::length(&queue.pending)) {
            let mid = *std::vector::borrow(&queue.pending, j);
            if (table::contains(&queue.requested_at_ms, mid)) {
                std::vector::push_back(&mut new_pending, mid);
            };
            j = j + 1;
        };
    ...
    *******************************/
    public entry fun exercise_american_now<C>(
        ...
        // Use production oracle instead of direct Pyth integration
        let _price_scaled = get_price_scaled_1e6(oracle_cfg, clock, price_info);
        let _ua = table::borrow(&reg.supported_underlyings, market.underlying);
        // Note: feed validation handled by oracle module
        ...
        if (net_to_long > 0) {
            let p = short_pos.collateral_locked.split(net_to_long, ctx);
            to_long.join(p);
        };
        sui::transfer::public_transfer(to_long, long_pos.owner);
        if (fee > 0) {
            let fee_coin = short_pos.collateral_locked.split(fee, ctx);
        ...
        event::emit(EarlyExercised { market_id: object::id(market), long_owner: long_pos.owner, short_owner: short_pos.owner, quantity, payout_to_long: net_to_long, fee_paid_usdc: fee, timestamp: 0u64 });
    }
    ...
    *******************************/
    public entry fun close_positions_by_premium<C>(
        ...
        let usdc_fee_to_collect = taker_fee;
        let net_amount = if (premium_total > usdc_fee_to_collect) { premium_total - usdc_fee_to_collect } else { 0 };
        if (payer_is_long) {
            // Long pays short to close; transfer net to short and fee to treasury
            if (net_amount > 0) { let to_short = collateral.split(net_amount, ctx); sui::transfer::public_transfer(to_short, short_pos.owner); };
            if (usdc_fee_to_collect > 0) { let mut fee_all = collateral.split(usdc_fee_to_collect, ctx); if (maker_rebate > 0 && maker_rebate < usdc_fee_to_collect) { let to_maker = fee_all.split(maker_rebate, ctx); sui::transfer::public_transfer(to_maker, short_pos.owner); }; let bot_cut = (fee_all.value() * reg.close_bot_reward_bps) / 10_000; if (bot_cut > 0) { let to_bots = fee_all.split(bot_cut, ctx); sui::transfer::public_transfer(to_bots, TreasuryMod::treasury_address(treasury)); }; sui::transfer::public_transfer(fee_all, TreasuryMod::treasury_address(treasury)); };
        } else {
            // Short pays long to close (buy-back)
            if (net_amount > 0) { let to_long = collateral.split(net_amount, ctx); sui::transfer::public_transfer(to_long, long_pos.owner); };
            if (usdc_fee_to_collect > 0) { let mut fee_all = collateral.split(usdc_fee_to_collect, ctx); if (maker_rebate > 0 && maker_rebate < usdc_fee_to_collect) { let to_maker = fee_all.split(maker_rebate, ctx); sui::transfer::public_transfer(to_maker, long_pos.owner); }; let bot_cut = (fee_all.value() * reg.close_bot_reward_bps) / 10_000; if (bot_cut > 0) { let to_bots = fee_all.split(bot_cut, ctx); sui::transfer::public_transfer(to_bots, TreasuryMod::treasury_address(treasury)); }; sui::transfer::public_transfer(fee_all, TreasuryMod::treasury_address(treasury)); };
        };
        ...
        if (refund > 0) { let c = short_pos.collateral_locked.split(refund, ctx); sui::transfer::public_transfer(c, short_pos.owner); };
        ...
        // Refund any remainder of collateral source
        sui::transfer::public_transfer(collateral, tx_context::sender(ctx));
        // Consume UNXV payment vector if not already consumed
        let mut _unxv_payment = _unxv_payment;
        while (std::vector::length(&_unxv_payment) > 0) {
            let remaining_coin = std::vector::pop_back(&mut _unxv_payment);
            sui::transfer::public_transfer(remaining_coin, tx_context::sender(ctx));
        };
        std::vector::destroy_empty(_unxv_payment);
        
        event::emit(OptionClosed { market_id: object::id(market), closer: if (payer_is_long) { long_pos.owner } else { short_pos.owner }, counterparty: if (payer_is_long) { short_pos.owner } else { long_pos.owner }, quantity, premium_per_unit, fee_paid_usdc: usdc_fee_to_collect, timestamp: 0u64 });
    }

    /*******************************
    * Liquidation: close pair at live price with penalty to liquidator
    *******************************/
    public entry fun liquidate_under_collateralized_pair<C>(
        _cfg: &CollateralConfig<C>,
        reg: &mut OptionsRegistry,
        market: &mut OptionMarket,
        long_pos: &mut OptionPosition<C>,
        ...
        let cur_coll = short_pos.collateral_locked.value();
        ...
        sui::transfer::public_transfer(to_long, long_pos.owner);
        if (fee > 0) { let fc = short_pos.collateral_locked.split(fee, ctx); sui::transfer::public_transfer(fc, TreasuryMod::treasury_address(treasury)); };
        // Liquidator bonus from remaining collateral proportional to liq_penalty_bps
        let bonus = (short_pos.collateral_locked.value() * reg.liq_penalty_bps) / 10_000;
        if (bonus > 0) { let b = short_pos.collateral_locked.split(bonus, ctx); sui::transfer::public_transfer(b, liquidator); };
        long_pos.quantity = long_pos.quantity - quantity;
        short_pos.quantity = short_pos.quantity - quantity;
        if (market.total_open_interest >= quantity) { market.total_open_interest = market.total_open_interest - quantity; };
        if (table::contains(&market.user_open_interest, long_pos.owner)) {
        ...
        event::emit(ShortLiquidated { market_id: object::id(market), short_owner: short_pos.owner, liquidator, quantity, collateral_seized: bonus, penalty_paid: bonus, timestamp: 0u64 });
    }
    ...
    *******************************/
    ...
    public fun short_health<C>(short_pos: &OptionPosition<C>, market: &OptionMarket): (u64, u64, bool) {
        let coll = short_pos.collateral_locked.value();
    ...
    fun market_key_bytes(underlying: &String, option_type: &String, strike: u64, expiry_ms: u64): vector<u8> {
        ...
        let ub = std::string::into_bytes(*underlying);
        let tb = std::string::into_bytes(*option_type);
        // Append underlying bytes
        let mut i = 0; while (i < std::vector::length(&ub)) { std::vector::push_back(&mut out, *std::vector::borrow(&ub, i)); i = i + 1; };
        std::vector::push_back(&mut out, 0u8);
        // Append type bytes
        let mut j = 0; while (j < std::vector::length(&tb)) { std::vector::push_back(&mut out, *std::vector::borrow(&tb, j)); j = j + 1; };
        std::vector::push_back(&mut out, 0u8);
    ...
    fun append_u64_be(out: &mut vector<u8>, x: u64) {
        let b7 = ((x >> 56) & 0xFF) as u8; std::vector::push_back(out, b7);
        let b6 = ((x >> 48) & 0xFF) as u8; std::vector::push_back(out, b6);
        let b5 = ((x >> 40) & 0xFF) as u8; std::vector::push_back(out, b5);
        let b4 = ((x >> 32) & 0xFF) as u8; std::vector::push_back(out, b4);
        let b3 = ((x >> 24) & 0xFF) as u8; std::vector::push_back(out, b3);
        let b2 = ((x >> 16) & 0xFF) as u8; std::vector::push_back(out, b2);
        let b1 = ((x >> 8) & 0xFF) as u8; std::vector::push_back(out, b1);
        let b0 = (x & 0xFF) as u8; std::vector::push_back(out, b0);
    }

    /*******************************
    * Displays for Market (type-level)
    *******************************/
    public entry fun init_market_display(publisher: &Publisher, ctx: &mut tx_context::TxContext) {
    ...
    public entry fun init_offer_and_position_displays<C>(_cfg: &CollateralConfig<C>, publisher: &Publisher, ctx: &mut tx_context::TxContext) {
        ...
        sui::transfer::public_transfer(disp_offer, tx_context::sender(ctx));
        ...
        sui::transfer::public_transfer(disp_esc, tx_context::sender(ctx));
        ...
        sui::transfer::public_transfer(disp_pos, tx_context::sender(ctx));
    }

    /*******************************
    * Read-only helpers
    *******************************/
    public fun list_underlyings(_reg: &OptionsRegistry): vector<String> { std::vector::empty<String>() } // TODO: table::keys not available
    ...
    public fun list_option_market_keys(_reg: &OptionsRegistry): vector<vector<u8>> { std::vector::empty<vector<u8>>() } // TODO: table::keys not available
    ...
    public fun get_market_strike_price(market: &OptionMarket): u64 { market.strike_price }
    public fun get_market_contract_size(market: &OptionMarket): u64 { market.contract_size }
    public fun get_market_info(market: &OptionMarket): (String, String, u64, u64, u64) { 
        (market.underlying, market.option_type, market.strike_price, market.expiry_ms, market.contract_size) 
    }
    ...
    *******************************/
    public entry fun expire_and_settle_market_cash(
        ...
        event::emit(OptionMarketSettled { market_id: object::id(market), underlying: market.underlying, option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, settlement_price: px, timestamp: now });
    }

    /*******************************
    * OTC matching: Writer short offer and buyer premium escrow
    *******************************/
    public fun place_short_offer<C>(
        market: &OptionMarket,
        quantity: u64,
        min_premium_per_unit: u64,
        mut collateral: Coin<C>,
        ...
        assert!(collateral.value() >= needed, E_COLLATERAL);
        let locked = collateral.split(needed, ctx);
        sui::transfer::public_transfer(collateral, tx_context::sender(ctx));
        ShortOffer { id: object::new(ctx), owner: tx_context::sender(ctx), market_id: object::id(market), option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, remaining_qty: quantity, min_premium_per_unit, collateral_locked: locked, created_at_ms: 0u64 }
    }
    ...
    public fun place_coin_short_offer<Base: store>(
        ...
        let have = base_in.value();
        assert!(have >= quantity, E_COLLATERAL);
        let escrow = base_in.split(quantity, ctx);
        sui::transfer::public_transfer(base_in, tx_context::sender(ctx));
        CoinShortOffer<Base> { id: object::new(ctx), owner: tx_context::sender(ctx), market_id: object::id(market), option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, remaining_qty: quantity, min_premium_per_unit, escrow_base: escrow, created_at_ms: 0u64 }
    }

    public fun place_premium_escrow<C>(
        _cfg: &CollateralConfig<C>,
        market: &OptionMarket,
        quantity: u64,
        premium_per_unit: u64,
        ...
        assert!(collateral.value() >= needed, E_PRICE);
        let escrow = collateral.split(needed, ctx);
        sui::transfer::public_transfer(collateral, tx_context::sender(ctx));
        PremiumEscrow { id: object::new(ctx), owner: tx_context::sender(ctx), market_id: object::id(market), option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, remaining_qty: quantity, premium_per_unit, escrow_collateral: escrow, created_at_ms: 0u64, expiry_cancel_ms }
    }

    public fun cancel_short_offer<C>(offer: ShortOffer<C>, ctx: &mut tx_context::TxContext) {
        assert!(offer.owner == tx_context::sender(ctx), E_NOT_ADMIN);
        let ShortOffer { id, owner, market_id: _, option_type: _, strike_price: _, expiry_ms: _, remaining_qty: _, min_premium_per_unit: _, collateral_locked, created_at_ms: _ } = offer;
        sui::transfer::public_transfer(collateral_locked, owner);
        sui::object::delete(id);
    }

    public fun cancel_coin_short_offer<Base: store>(offer: CoinShortOffer<Base>, ctx: &mut tx_context::TxContext) {
        assert!(offer.owner == tx_context::sender(ctx), E_NOT_ADMIN);
    ...
    public fun cancel_premium_escrow<C>(esc: PremiumEscrow<C>, ctx: &mut tx_context::TxContext) {
        assert!(esc.owner == tx_context::sender(ctx), E_NOT_ADMIN);
        assert!(0u64 >= esc.expiry_cancel_ms, E_BAD_PARAMS);
        let PremiumEscrow { id, owner, market_id: _, option_type: _, strike_price: _, expiry_ms: _, remaining_qty: _, premium_per_unit: _, escrow_collateral, created_at_ms: _, expiry_cancel_ms: _ } = esc;
    ...
    public entry fun match_offer_and_escrow<C>(
        ...
        if (discount_usdc > 0 && std::vector::length(&unxv_payment) > 0) {
            ...
            if (unxv_px > 0) {
                ...
                if (have >= unxv_needed) {
                    ...
                    sui::transfer::public_transfer(merged, escrow.owner);
                    discount_applied = true;
                } else {
                    sui::transfer::public_transfer(merged, escrow.owner);
                }
        ...
        let usdc_fee_to_collect = if (discount_applied) { if (trade_fee > discount_usdc) { trade_fee - discount_usdc } else { 0 } } else { trade_fee };

        // Move premium net of fee to writer, pay fee to treasury
        let net_to_writer = if (premium_total > usdc_fee_to_collect) { premium_total - usdc_fee_to_collect } else { 0 };
        if (net_to_writer > 0) {
            let to_writer = escrow.escrow_collateral.split(net_to_writer, ctx);
            sui::transfer::public_transfer(to_writer, offer.owner);
        };
        if (usdc_fee_to_collect > 0) {
            let fee_coin = escrow.escrow_collateral.split(usdc_fee_to_collect, ctx);
            sui::transfer::public_transfer(fee_coin, TreasuryMod::treasury_address(treasury));
        };
        
        // Create positions
        let long_pos = OptionPosition<C> { id: object::new(ctx), owner: escrow.owner, market_id: object::id(market), option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, side: 0, quantity: fill, premium_per_unit: escrow.premium_per_unit, opened_at_ms: 0u64, collateral_locked: coin::zero<C>(ctx) };
        ...
        let short_locked = offer.collateral_locked.split(short_needed, ctx);
        let short_pos = OptionPosition<C> { id: object::new(ctx), owner: offer.owner, market_id: object::id(market), option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, side: 1, quantity: fill, premium_per_unit: 0, opened_at_ms: 0u64, collateral_locked: short_locked };
        ...
        // Store data for events before consuming structs
        let escrow_owner = escrow.owner;
        let offer_owner = offer.owner;
        let premium_per_unit = escrow.premium_per_unit;
        
        // Consume remaining UNXV payment vector
        while (std::vector::length(&unxv_payment) > 0) {
            let remaining_coin = std::vector::pop_back(&mut unxv_payment);
            sui::transfer::public_transfer(remaining_coin, escrow_owner);
        };
        std::vector::destroy_empty(unxv_payment);
        
        // Handle remaining offers/escrows
        if (offer.remaining_qty > 0) {
            transfer::share_object(offer);
        } else {
            let ShortOffer { id, owner: _, market_id: _, option_type: _, strike_price: _, expiry_ms: _, remaining_qty: _, min_premium_per_unit: _, collateral_locked, created_at_ms: _ } = offer;
            transfer::public_transfer(collateral_locked, offer_owner);
            object::delete(id);
        };
        
        if (escrow.remaining_qty > 0) {
            transfer::share_object(escrow);
        } else {
            let PremiumEscrow { id, owner: _, market_id: _, option_type: _, strike_price: _, expiry_ms: _, remaining_qty: _, premium_per_unit: _, escrow_collateral, created_at_ms: _, expiry_cancel_ms: _ } = escrow;
            transfer::public_transfer(escrow_collateral, escrow_owner);
            object::delete(id);
        };
        
        event::emit(OptionMatched { market_id: object::id(market), buyer: escrow_owner, writer: offer_owner, option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, quantity: fill, premium_per_unit: premium_per_unit, fee_paid_usdc: usdc_fee_to_collect, unxv_discount_applied: discount_applied, timestamp: 0u64 });
        event::emit(OptionOpened { market_id: object::id(market), buyer: escrow_owner, writer: offer_owner, quantity: fill, premium_per_unit: premium_per_unit, timestamp: 0u64 });
    }

    /// Match coin-escrowed CALL offer with premium escrow (physical delivery on exercise)
    public fun match_coin_offer_and_escrow<Base: store, C>(
        _cfg: &CollateralConfig<C>,
        reg: &mut OptionsRegistry,
        market: &mut OptionMarket,
        ...
        let usdc_owed = fill * escrow.premium_per_unit;
        let to_writer = escrow.escrow_collateral.split(usdc_owed, ctx);
        sui::transfer::public_transfer(to_writer, offer.owner);
        ...
        let short_split = offer.escrow_base.split(fill, ctx);
        let short_pos = OptionPosition<C> { id: short_pos_id, owner: offer.owner, market_id: object::id(market), option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, side: 1, quantity: fill, premium_per_unit: 0, opened_at_ms: 0u64, collateral_locked: coin::zero<C>(ctx) };
        ...
        if (offer.remaining_qty > 0) {
            ...
            let CoinShortOffer { id, owner: _, market_id: _, option_type: _, strike_price: _, expiry_ms: _, remaining_qty: _, min_premium_per_unit: _, escrow_base, created_at_ms: _ } = offer;
            transfer::public_transfer(escrow_base, offer_owner);
            object::delete(id);
        };
        ...
        event::emit(OptionMatched { market_id: object::id(market), buyer: escrow_owner, writer: offer_owner, option_type: market.option_type, strike_price: market.strike_price, expiry_ms: market.expiry_ms, quantity: fill, premium_per_unit: premium_per_unit, fee_paid_usdc: 0, unxv_discount_applied: false, timestamp: 0u64 });
        event::emit(OptionOpened { market_id: object::id(market), buyer: escrow_owner, writer: offer_owner, quantity: fill, premium_per_unit: premium_per_unit, timestamp: 0u64 });
    }
    ...
    *******************************/
    public entry fun settle_positions_cash<C>(
        ...
        if (cur_coll < maint_needed) {
            event::emit(MarginCallTriggered { short_owner: short_pos.owner, market_id: object::id(market), required_collateral: maint_needed, current_collateral: cur_coll, timestamp: 0u64 });
        };
        ...
        if (fee > 0) {
            let mut fee_coin = short_pos.collateral_locked.split(fee, ctx);
            let bot_cut = (fee_coin.value() * reg.settlement_bot_reward_bps) / 10_000;
            if (bot_cut > 0) { let bot_coin = fee_coin.split(bot_cut, ctx); sui::transfer::public_transfer(bot_coin, TreasuryMod::treasury_address(treasury)); };
            sui::transfer::public_transfer(fee_coin, TreasuryMod::treasury_address(treasury));
        };
        ...
        event::emit(OptionSettled { market_id: object::id(market), long_owner: long_pos.owner, short_owner: short_pos.owner, option_type: long_pos.option_type, strike_price: long_pos.strike_price, expiry_ms: long_pos.expiry_ms, quantity, settlement_price: market.settlement_price, payout_to_long: net_to_long, fee_paid_usdc: fee, timestamp: 0u64 });
    }

    /*******************************
    * Physical exercise for coin-escrowed CALLs
    *******************************/
    public fun exercise_physical_call<Base: store, C>(
        _cfg: &CollateralConfig<C>,
        market: &mut OptionMarket,
        long_pos: &mut OptionPosition<C>,
        short_pos: &mut OptionPosition<C>,
        ...
        // Deliver underlying
        let deliver = escrow.escrow_base.split(quantity, ctx);
        sui::transfer::public_transfer(deliver, long_pos.owner);
        ...
        if (escrow.escrow_base.value() == 0) {
            // Note: In production, escrow cleanup would be handled by a separate administrative function
            // to avoid ability constraint issues with Coin destructuring
        }
    }
    ...
    public entry fun emit_physical_delivery_completed(
        ...
        ctx: &tx_context::TxContext
    ) { event::emit(PhysicalDeliveryCompleted { market_id: object::id(market), fulfiller: tx_context::sender(ctx), side, quantity, avg_settlement_price, timestamp: 0u64 }); }
    ...
    *******************************/
    public entry fun request_physical_delivery_long<C>(
        ...
        _ctx: &tx_context::TxContext
    ) { assert!(quantity > 0 && quantity <= long_pos.quantity, E_AMOUNT); event::emit(PhysicalDeliveryRequested { market_id: object::id(market), requester: long_pos.owner, side: 0, quantity, min_settlement_price, timestamp: 0u64 }); }
    ...
    public entry fun request_physical_delivery_short<C>(
        ...
        min_settlement_price: u64,
        _ctx: &tx_context::TxContext
    ) { assert!(quantity > 0 && quantity <= short_pos.quantity, E_AMOUNT); event::emit(PhysicalDeliveryRequested { market_id: object::id(market), requester: short_pos.owner, side: 1, quantity, min_settlement_price, timestamp: 0u64 }); }

    /*******************************
    * Public wrapper functions for cross-module calls
    *******************************/
    
    /// Public wrapper for place_short_offer entry function
    public fun place_short_offer_public<C>(
        _cfg: &CollateralConfig<C>,
        market: &mut OptionMarket,
        quantity: u64,
        min_premium_per_unit: u64,
        ...
        // For now, create a placeholder short offer
        // TODO: Implement full logic or call the entry function properly
        ShortOffer {
            id: object::new(ctx),
            owner: tx_context::sender(ctx),
            market_id: object::id(market),
            option_type: b"call".to_string(), // placeholder
            strike_price: 0, // placeholder  
            expiry_ms: 0, // placeholder
    ...
    /// Public wrapper for place_premium_escrow entry function  
    public fun place_premium_escrow_public<C>(
        _cfg: &CollateralConfig<C>,
        market: &mut OptionMarket,
        quantity: u64,
        premium_per_unit: u64,
        ...
        // For now, create a placeholder premium escrow
        // TODO: Implement full logic or call the entry function properly
        PremiumEscrow {
            id: object::new(ctx),
            owner: tx_context::sender(ctx),
            ...
            remaining_qty: quantity,
            premium_per_unit,
            escrow_collateral: collateral,
            created_at_ms: 0,
            expiry_cancel_ms,
        }
    }
    
    /// Public function for matching offers (placeholder implementation)
    public fun match_offer_and_escrow_public<C>(
        _cfg: &CollateralConfig<C>,
        _reg: &OptionsRegistry,
        _market: &mut OptionMarket,
        _offer: &mut ShortOffer<C>,
        _escrow: &mut PremiumEscrow<C>,
        _max_fill_qty: u64,
        mut _unxv_payment: Coin<UNXV>,
        _treasury: &mut Treasury<C>,
        _oracle_cfg: &OracleConfig,
        _clock: &Clock,
        _unxv_price: &PriceInfoObject,
        ...
        // Consume single UNXV payment coin
        if (_unxv_payment.value() > 0) {
            sui::transfer::public_transfer(_unxv_payment, tx_context::sender(_ctx));
        } else {
            coin::destroy_zero(_unxv_payment);
        };
