...
module unxversal::ubtc {
    ...
    public struct UBTC has drop {}

    public struct Faucet has key, store { id: UID, cap: TreasuryCap<UBTC>, usdu_treasury: Balance<USDU>, paused: bool }
    ...
    public struct BoughtWithUSDU has copy, drop { buyer: address, usdu_in: u64, ubtc_out: u64, price_1e6: u64, timestamp_ms: u64 }

    fun init(witness: UBTC, ctx: &mut TxContext) {
        let (cap, metadata) = coin::create_currency(witness, 6, b"UBTC", b"Unxversal Testnet BTC", b"", option::none(), ctx);
    ...
    public fun set_paused(reg_admin: &AdminRegistry, faucet: &mut Faucet, paused: bool, clock: &Clock, ctx: &TxContext) {
        ...
        faucet.paused = paused; event::emit(Paused { paused, by: ctx.sender(), timestamp_ms: sui::clock::timestamp_ms(clock) });
    }

    public fun withdraw_usdu(reg_admin: &AdminRegistry, faucet: &mut Faucet, amount: u64, ctx: &mut TxContext): Coin<USDU> {
        assert!(AdminMod::is_admin(reg_admin, ctx.sender()), E_NOT_ADMIN);
        coin::from_balance(balance::split(&mut faucet.usdu_treasury, amount), ctx)
    }

    public fun buy_with_usdu(reg: &OracleRegistry, faucet: &mut Faucet, price_info_object: &PriceInfoObject, usdu_in: Coin<USDU>, min_out: u64, clock: &Clock, ctx: &mut TxContext): Coin<UBTC> {
        assert!(!faucet.paused, E_PAUSED);
        let price_1e6 = oracle::get_price_for_symbol(reg, clock, &symbol_btc_usdc(), price_info_object);
        ...
        event::emit(BoughtWithUSDU { buyer: ctx.sender(), usdu_in: usdu_amount, ubtc_out: out, price_1e6, timestamp_ms: sui::clock::timestamp_ms(clock) });
        minted
    }
    ...
    fun symbol_btc_usdc(): String { string::utf8(b"BTC/USDC") }
}
