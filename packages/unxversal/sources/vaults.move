...
module unxversal::vaults {
    ...
    use unxversal::synthetics::{Self as SynthMod, SynthRegistry, AdminCap, CollateralVault, SynthMarket, SynthEscrow};
    ...
    public fun init_manager_stake_registry(registry: &SynthRegistry, min_stake_unxv: u64, ctx: &mut TxContext) {
    ...
    public fun set_min_stake(registry: &SynthRegistry, rs: &mut ManagerStakeRegistry, min_stake_unxv: u64, ctx: &TxContext) { assert_is_admin(registry, ctx.sender()); rs.min_stake_unxv = min_stake_unxv }
    public fun pause_stake_registry(registry: &SynthRegistry, rs: &mut ManagerStakeRegistry, ctx: &TxContext) { assert_is_admin(registry, ctx.sender()); rs.paused = true }
    public fun resume_stake_registry(registry: &SynthRegistry, rs: &mut ManagerStakeRegistry, ctx: &TxContext) { assert_is_admin(registry, ctx.sender()); rs.paused = false }

    public fun stake_unxv(rs: &mut ManagerStakeRegistry, mut coins: vector<Coin<UNXV>>, ctx: &mut TxContext) {
        ...
        event::emit(StakeDeposited { manager: ctx.sender(), amount: amt, total_after, timestamp: sui::tx_context::epoch_timestamp_ms(ctx) });
    }
    public fun withdraw_stake(rs: &mut ManagerStakeRegistry, amount: u64, ctx: &mut TxContext): Coin<UNXV> {
        assert!(!rs.paused, E_REGISTRY_PAUSED);
        ...
        event::emit(StakeWithdrawn { manager: ctx.sender(), amount, total_after, timestamp: sui::tx_context::epoch_timestamp_ms(ctx) });
        out
    }
    public fun slash_stake(registry: &SynthRegistry, rs: &mut ManagerStakeRegistry, treasury: &mut Treasury<UNXV>, manager: address, amount: u64, ctx: &mut TxContext) {
        assert_is_admin(registry, ctx.sender());
        ...
        if (take > 0) {
            ...
            let total_after = balance::value(b);
            event::emit(StakeSlashed { manager, amount: take, total_after, timestamp: sui::tx_context::epoch_timestamp_ms(ctx) });
        }
    }
    public fun set_frozen(registry: &SynthRegistry, rs: &mut ManagerStakeRegistry, manager: address, frozen: bool, ctx: &TxContext) { assert_is_admin(registry, ctx.sender()); if (table::contains(&rs.frozen, manager)) { let _ = table::remove(&mut rs.frozen, manager); }; table::add(&mut rs.frozen, manager, frozen); event::emit(ManagerFrozen { manager, frozen, timestamp: sui::tx_context::epoch_timestamp_ms(ctx) }) }
    ...
    public fun create_vault<BaseUSD: store>(rs: &mut ManagerStakeRegistry, min_cash_bps: u64, ctx: &mut TxContext) {
        ...
        let v = Vault<BaseUSD> {
            ...
            frozen: false, created_at_ms: sui::tx_context::epoch_timestamp_ms(ctx)
        };
    ...
    public fun set_vault_frozen<BaseUSD: store>(rs: &ManagerStakeRegistry, v: &mut Vault<BaseUSD>, frozen: bool, ctx: &TxContext) { assert!(v.manager == ctx.sender(), E_NOT_MANAGER); assert_manager_active(rs, v.manager); v.frozen = frozen; event::emit(VaultFrozen { vault_id: object::id(v), frozen, timestamp: sui::tx_context::epoch_timestamp_ms(ctx) }) }
    ...
    // Admin knobs for caps/fees
    public fun set_vault_caps_and_fees<BaseUSD: store>(registry: &SynthRegistry, v: &mut Vault<BaseUSD>, max_aum_base: u64, perf_fee_bps: u64, _admin: &AdminCap, ctx: &TxContext) {
        assert_is_admin(registry, ctx.sender());
    ...
    public fun deposit_asset<Asset: store, BaseUSD: store>(rs: &ManagerStakeRegistry, v: &Vault<BaseUSD>, store: &mut VaultAssetStore<Asset, BaseUSD>, amt: Coin<Asset>, _ctx: &TxContext) { assert_manager_active(rs, v.manager); assert!(v.manager == _ctx.sender() && !v.frozen && store.vault_id == object::id(v), E_NOT_MANAGER); coin::join(&mut store.coin, amt) }
