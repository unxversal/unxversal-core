...
module unxversal::big_vector;

use sui::dynamic_field as df;

use fun sui::object::new as TxContext.new;

// Local helpers (ported to avoid external utils dependency)
fun pop_until<T>(v: &mut vector<T>, n: u64): vector<T> {
    let mut res = vector[];
    while (v.length() > n) {
...
fun pop_n<T>(v: &mut vector<T>, n: u64): vector<T> {
    let mut res = vector[];
...
/// A large, ordered map-like vector backed by a B+ tree of dynamic fields.
...
/// Check whether `key` exists in `self` without aborting.
public fun contains_key<E: store>(self: &BigVector<E>, key: u128): bool {
    if (self.root_id == NO_SLICE) { return false };
    let (_, leaf, off) = self.find_leaf(key);
    if (off >= leaf.keys.length()) { false } else { key == leaf.keys[off] }
}
...
public fun borrow<E: store>(self: &BigVector<E>, ix: u128): &E {
    let (r, off) = self.slice_around(ix);
    let slice = self.borrow_slice(r);
    slice_borrow(slice, off)
}

/// Access the element at index `ix` in `self`, mutably.
public fun borrow_mut<E: store>(self: &mut BigVector<E>, ix: u128): &mut E {
    let (r, off) = self.slice_around(ix);
    let slice = self.borrow_slice_mut(r);
    slice_borrow_mut(slice, off)
}
...
public fun remove<E: store>(self: &mut BigVector<E>, key: u128): E {
    ...
    if (rm_fix == RM_FIX_EMPTY) {
        if (self.depth == 0) {
            let Slice<E> { prev: _, next: _, keys: _, vals } = df::remove(&mut self.id, root_id);
            vals.destroy_empty();
...
public fun borrow_slice<E: store>(self: &BigVector<E>, r: SliceRef): &Slice<E> {
    df::borrow(&self.id, r.ix)
}
...
public fun borrow_slice_mut<E: store>(
    ...
    df::borrow_mut(&mut self.id, r.ix)
}
...
public fun slice_borrow_mut<E: store>(self: &mut Slice<E>, ix: u64): &mut E {
    &mut self.vals[ix]
}
...
fun singleton<E: store>(key: u128, val: E): Slice<E> {
    Slice { prev: NO_SLICE, next: NO_SLICE, keys: vector[key], vals: vector[val] }
}

/// Create a slice representing an interior node containing a
/// single branch.
fun branch(key: u128, left: u64, right: u64): Slice<u64> {
    Slice { prev: NO_SLICE, next: NO_SLICE, keys: vector[key], vals: vector[left, right] }
}
...
fun find_max_leaf<E: store>(self: &BigVector<E>): (u64, &Slice<E>, u64) {
    ...
    while (depth > 0) {
        let slice: &Slice<u64> = df::borrow(&self.id, slice_id);
        slice_id = slice.vals[slice.keys.length()];
        depth = depth - 1;
    };
...
fun leaf_insert<E: store>(self: &mut BigVector<E>, slice_id: u64, key: u128, val: E): (u128, u64) {
    ...
    if (off < leaf.keys.length() && key == leaf.keys[off]) {
        abort EExists
    };

    // If there is enough space in the current leaf, no need to split.
    ...
    let mut next = Slice { prev: slice_id, next: leaf.next, keys: pop_until(&mut leaf.keys, split_at), vals: pop_until(&mut leaf.vals, split_at) };

    // Insert the key-value pair into the correct side of the split -- the first element in the new slice is the pivot.
...
fun node_insert<E: store>(
    ...
    let mut next = Slice { prev: slice_id, next: node.next, keys: pop_until(&mut node.keys, split_at), vals: pop_until(&mut node.vals, split_at) };
...
///
/// `prev_id` and `next_id` are siblings at the same level for redistribution/merge.
fun slice_remove<E: store>(
    ...
    if (depth == 0) {
        self.leaf_remove(prev_id, prev_key, slice_id, next_key, next_id, key)
    } else {
        self.node_remove(prev_id, prev_key, slice_id, next_key, next_id, depth - 1, key)
    }
}

/// Like `slice_remove` but leaf.
fun leaf_remove<E: store>(
    ...
    if (prev_id != NO_SLICE) {
        ...
        if (prev.vals.length() > min_slice_size) {
            return (val, RM_FIX_STEAL_L, self.slice_redistribute<E, E>(prev_id, prev_key, slice_id))
        }
    };
    ...
    if (next_id != NO_SLICE) {
        ...
        if (next.vals.length() > min_slice_size) {
            return (val, RM_FIX_STEAL_R, self.slice_redistribute<E, E>(slice_id, next_key, next_id))
        }
    };
    ...
    // Neither neighbour exists, must be the root -- check empty.
    if (remaining == 0) { (val, RM_FIX_EMPTY, 0) } else { (val, RM_FIX_NOTHING, 0) }
}

/// Like `slice_remove` but interior node.
fun node_remove<E: store>(
    ...
    let (child_prev_id, child_prev_key) = if (off == 0) { (NO_SLICE, 0) } else (node.vals[off - 1], node.keys[off - 1]);
    let (child_next_id, child_next_key) = if (off == node.keys.length()) { (NO_SLICE, 0) } else (node.vals[off + 1], node.keys[off]);

    let (val, rm_fix, pivot) = self.slice_remove(child_prev_id, child_prev_key, child_id, child_next_key, child_next_id, depth, key);

    // Re-borrow node mutably after recursive call, to perform fix-ups.
    let node: &mut Slice<u64> = df::borrow_mut(&mut self.id, slice_id);
    ...
    if (prev_id != NO_SLICE) {
        ...
        if (prev.vals.length() > min_fan_out) {
            return (val, RM_FIX_STEAL_L, self.slice_redistribute<E, u64>(prev_id, prev_key, slice_id))
        }
    };
    ...
    if (next_id != NO_SLICE) {
        ...
        if (next.vals.length() > min_fan_out) {
            return (val, RM_FIX_STEAL_R, self.slice_redistribute<E, u64>(slice_id, next_key, next_id))
        }
    };
    ...
    // Root interior node: empty=error, 1 child=can be replaced.
    if (remaining == 0) {
        abort EBadRemove
    } else if (remaining == 1) {
        (val, RM_FIX_EMPTY, 0)
...
/// Redistribute the elements in `left_id` and `right_id` separated by `pivot`.
/// Returns the new pivot element between the two slices. Aborts if not adjacent.
fun slice_redistribute<E: store, F: store>(
    ...
    // Remove slices to manipulate both simultaneously.
    ...
    let Slice { prev: lprev, next: lnext, keys: mut lkeys, vals: mut lvals } = left;
    let Slice { prev: rprev, next: rnext, keys: rkeys, vals: rvals } = right;
    ...
    // Direction detection
    let left_to_right = if (new_l_len < old_l_len) { true } else if (new_r_len < old_r_len) { false } else { abort EBadRedistribution };

    // Redistribute values
    let (lvals, rvals) = if (left_to_right) {
        let mut mvals = pop_until(&mut lvals, new_l_len);
        mvals.append(rvals);
        (lvals, mvals)
    } else {
        let mut mvals = rvals;
        let rvals = pop_n(&mut mvals, new_r_len);
        lvals.append(mvals);
        (lvals, rvals)
    };
    ...
    let (lkeys, pivot, rkeys) = if (is_leaf && left_to_right) {
        let mut mkeys = pop_until(&mut lkeys, new_l_len);
        let pivot = mkeys[0];
        mkeys.append(rkeys);
        (lkeys, pivot, mkeys)
    } else if (is_leaf && !left_to_right) {
        let mut mkeys = rkeys;
        let rkeys = pop_n(&mut mkeys, new_r_len);
        let pivot = rkeys[0];
        lkeys.append(mkeys);
        (lkeys, pivot, rkeys)
    } else if (!is_leaf && left_to_right) {
        ...
        let mut mkeys = rkeys;
        let rkeys = pop_n(&mut mkeys, new_r_len - 1);
    ...
    // Add slices back
    df::add(&mut self.id, left_id, Slice { prev: lprev, next: lnext, keys: lkeys, vals: lvals });
    df::add(&mut self.id, right_id, Slice { prev: rprev, next: rnext, keys: rkeys, vals: rvals });

    pivot
}

/// Merge the `right_id` slice into `left_id` (adjacent slices), separated by `pivot`.
...
public fun inorder_values<E: store + copy>(self: &BigVector<E>): vector<vector<E>> {
    let mut vals = vector[];
    if (self.root_id == NO_SLICE) { return vals };
    let (mut slice_id, mut depth) = (self.root_id, self.depth);
    while (depth > 0) {
        let slice: &Slice<u64> = df::borrow(&self.id, slice_id);
        slice_id = slice.vals[0];
...
#[test_only]
/// Destroy `self`, even if it contains elements, as long as they are droppable.
public fun drop<E: store + drop>(self: BigVector<E>) {
    let BigVector { mut id, depth, length: _, max_slice_size: _, max_fan_out: _, root_id, last_id: _ } = self;
