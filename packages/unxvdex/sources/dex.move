...
/// - Provide swap helpers usable by UI with combined (taker+maker) protocol fees
///
/// Notes:
/// - This module defers matching and settlement to deepbook::pool
/// - Protocol fee is applied to taker flows of swap helpers and on order placement at injection paths
module unxvdex::dex {
    use sui::{
    ...
    // Unxversal
    use unxvcore::unxv::UNXV;
    use unxvcore::admin::{Self as AdminMod, AdminRegistry};
    use unxvcore::fees::{Self as fees, FeeConfig, FeeVault};
    use unxvcore::staking::{Self as staking, StakingPool};
    use unxvcore::rewards::{Self as rewards, Rewards};
    ...
    /// We optionally assess a protocol fee via separate helpers; this function delegates directly to DeepBook.
    ...
    /// Place a market order with Unxversal fee handling; delegates to DeepBook.
    ...
    public fun place_limit_order_with_protocol_fee_bid<Base, Quote>(
        ...
        assert!(quantity > 0, E_ZERO_AMOUNT);
        // notional_quote = quantity * price (DeepBook units). Do math in u128 to avoid overflow.
        ...
        let fee_amt = (((quantity as u128) * (price as u128) * (maker_eff as u128)) / (fees::bps_denom() as u128)) as u64;
        ...
        if (option::is_some(&maybe_unxv)) {
            ...
            event::emit(ProtocolFeeTaken { payer: ctx.sender(), base_fee_asset_unxv: true, amount: fee_amt, asset: type_name::with_defining_ids<Quote>(), timestamp_ms: sui::clock::timestamp_ms(clock) });
        } else {
            event::emit(ProtocolFeeTaken { payer: ctx.sender(), base_fee_asset_unxv: false, amount: fee_amt, asset: type_name::with_defining_ids<Quote>(), timestamp_ms: sui::clock::timestamp_ms(clock) });
        };
    ...
    public fun place_limit_order_with_protocol_fee_ask<Base, Quote>(
        ...
        if (option::is_some(&maybe_unxv)) {
            let unxv = option::extract(&mut maybe_unxv);
            transfer::public_transfer(unxv, ctx.sender());
            event::emit(ProtocolFeeTaken { payer: ctx.sender(), base_fee_asset_unxv: true, amount: fee_amt, asset: type_name::with_defining_ids<Base>(), timestamp_ms: sui::clock::timestamp_ms(clock) });
        } else {
            event::emit(ProtocolFeeTaken { payer: ctx.sender(), base_fee_asset_unxv: false, amount: fee_amt, asset: type_name::with_defining_ids<Base>(), timestamp_ms: sui::clock::timestamp_ms(clock) });
        };
        option::destroy_none(maybe_unxv);
    ...
    public fun place_market_order_with_protocol_fee_bid<Base, Quote>(
        ...
        let fee_amt = (((quantity as u128) * (mid as u128) * (maker_eff as u128)) / (fees::bps_denom() as u128)) as u64;
        let paid = coin::value(&fee_payment_quote);
        assert!(paid >= fee_amt, E_INJECTION_FEE_NOT_PAID);
        let fee_coin = coin::split(&mut fee_payment_quote, fee_amt, ctx);
        fees::route_fee<Quote>(vault, fee_coin, clock, ctx);
    ...
    // === UI swap wrapper with combined maker+taker protocol fee (input-token backend) ===
    /// UI-friendly swap wrapper that charges combined protocol fees (maker+ taker) after discounts.
    /// Only one of base_in or quote_in should be non-zero.
    /// Returns the two relevant coins and transfers any DEEP change back to the caller.
    public fun swap_exact_quantity_ui<Base, Quote>(
        pool: &mut Pool<Base, Quote>,
        cfg: &FeeConfig,
        vault: &mut FeeVault,
        staking_pool: &mut StakingPool,
        ...
        let is_bid = coin::value(&quote_in) > 0;
        assert!((coin::value(&base_in) > 0) != (coin::value(&quote_in) > 0), E_ZERO_AMOUNT);
        let (taker_eff, maker_eff) = fees::apply_discounts_dex(
            fees::dex_taker_fee_bps(cfg),
            fees::dex_maker_fee_bps(cfg),
            option::is_some(&maybe_unxv),
            staking_pool,
        ...
        let combined_bps = taker_eff + maker_eff;
        if (is_bid) {
            let q_amt = coin::value(&quote_in);
            let fee_amt = (q_amt as u128 * (combined_bps as u128) / (fees::bps_denom() as u128)) as u64;
            let fee_coin = coin::split(&mut quote_in, fee_amt, ctx);
            fees::route_fee<Quote>(vault, fee_coin, clock, ctx);
            if (option::is_some(&maybe_unxv)) {
                let unxv = option::extract(&mut maybe_unxv);
            ...
            option::destroy_none(maybe_unxv);
            let deep_zero = coin::zero<DEEP>(ctx);
            let (base_out, quote_left, deep_back) = db_pool::swap_exact_quote_for_base(pool, quote_in, deep_zero, min_out, clock, ctx);
            transfer::public_transfer(deep_back, ctx.sender());
            // Consume base_in in the bid branch (must be zero-value per assertion)
            coin::destroy_zero(base_in);
            (base_out, quote_left)
        } else {
            let b_amt = coin::value(&base_in);
            let fee_amt = (b_amt as u128 * (combined_bps as u128) / (fees::bps_denom() as u128)) as u64;
            let fee_coin = coin::split(&mut base_in, fee_amt, ctx);
            fees::route_fee<Base>(vault, fee_coin, clock, ctx);
            if (option::is_some(&maybe_unxv)) {
                let unxv = option::extract(&mut maybe_unxv);
            ...
            option::destroy_none(maybe_unxv);
            let deep_zero = coin::zero<DEEP>(ctx);
            let (base_left, quote_out, deep_back) = db_pool::swap_exact_base_for_quote(pool, base_in, deep_zero, min_out, clock, ctx);
            transfer::public_transfer(deep_back, ctx.sender());
            // Consume quote_in in the ask branch (must be zero-value per assertion)
            coin::destroy_zero(quote_in);
            (base_left, quote_out)
        }
    }
}
