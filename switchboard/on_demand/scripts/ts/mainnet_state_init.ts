...
tx.moveCall({
  target: `${ON_DEMAND_MAINNET_OBJECT_PACKAGE_ID}::set_package_id_action::run`,
  arguments: [
    tx.object(adminCap),
    tx.object(ON_DEMAND_MAINNET_STATE_OBJECT_ID),
    tx.pure.id(ON_DEMAND_MAINNET_OBJECT_PACKAGE_ID),
  ],
});

tx.moveCall({
  target: `${ON_DEMAND_MAINNET_OBJECT_PACKAGE_ID}::set_guardian_queue_id_action::run`,
  arguments: [
    tx.object(adminCap),
    tx.object(ON_DEMAND_MAINNET_STATE_OBJECT_ID),
...
tx.moveCall({
  target: `${ON_DEMAND_MAINNET_OBJECT_PACKAGE_ID}::set_oracle_queue_id_action::run`,
  arguments: [
    tx.object(adminCap),
    tx.object(ON_DEMAND_MAINNET_STATE_OBJECT_ID),
    tx.pure.id(queueId),
